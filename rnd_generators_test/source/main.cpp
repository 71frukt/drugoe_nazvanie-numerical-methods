...
double ChiSquareDistrTest(const std::vector<double>& data, double df, const rnd_generators_test::IFirstLevelTester& lvl1)
{
    ...
    for (double x : data) 
        u_data.push_back(boost::math::cdf(dist, x < 0 ? 0 : x));
    
    return RunTest(u_data, lvl1);
}

// Вывод только для непрерывных распределений (Hypercube)
void PrintContinuousResults(const std::string& group_name,
...
int main()
{
    ...
    const size_t block_size = 4096; // 2^12
    const size_t total_size = num_blocks * block_size;
    
    // Для битового теста Фурье требуется total_size битов
    const size_t total_bits = total_size; 
...

    {
        ...
        rnd_generators::MRG32k3a gen_bits(777); // Отдельный генератор для сырых битов
        ...
        PrintContinuousResults("CUSTOM (MRG32k3a Source)", c_unif, c_norm, c_chi, hypercube_test);
        
        std::vector<double> raw_bits = rnd_generators_test::utils::GenerateBitSequence(gen_bits, total_bits);
        RLSU_INFO("--- Fourier Test (Raw Bits from MRG32k3a) ---");
        RLSU_INFO("P-Value: {}", UniformityDistrTest(raw_bits, fourier_test));
        RLSU_INFO("");
    }

    {
        ...
        PrintContinuousResults("MIXED (MT19937 Source + Custom Algos)", m_unif, m_norm, m_chi, hypercube_test);
        RLSU_INFO("");
    }

    {
        ...
        PrintContinuousResults("STANDARD (MT19937 + STD Algos)", s_unif, s_norm, s_chi, hypercube_test);
        
        std::vector<double> raw_bits = rnd_generators_test::utils::GenerateBitSequence(engine_bits, total_bits);
        RLSU_INFO("--- Fourier Test (Raw Bits from MT19937) ---");
        RLSU_INFO("P-Value: {}", UniformityDistrTest(raw_bits, fourier_test));
        RLSU_INFO("");
    }
