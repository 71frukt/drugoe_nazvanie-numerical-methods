...
{
public:
    explicit ChiSqGenerator(double freedom_degree)
        : freedom_degree_(freedom_degree)
    {
        if (freedom_degree_ < 1.0)
        {
            RLSU_THROW<std::runtime_error>("degree of freedom < 1 in ChiGenerator ctor");
        }

        if (freedom_degree_ >= 2.0)
        {
            d_ = (freedom_degree_ / 2.0) - (1.0 / 3.0);
